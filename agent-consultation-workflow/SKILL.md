...
description: 不明点を外部AI（sub-agentやcopilotなど）への照会で解消するためのワークフロースキル。背景情報、AI Agentとしての立場、確認したい論点、制約を整理して一括質問を作成し、質問返しを避けて結論まで回収する。仕様が曖昧なとき、複数案の比較判断が必要なとき、一次情報が不足しているときに使用する。単なる雑談や自明な質問には使用しない。
---
