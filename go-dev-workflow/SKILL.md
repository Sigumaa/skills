...
description: Goプロジェクトの新規立ち上げと日常開発フローを標準化するスキル。go mod initによる初期化、ディレクトリ構成、実装、依存整理、go fmt/go vet/go testによる検証、コミット前チェックまでを案内する。Go言語で新規開発を始めるとき、既存Goリポジトリへ機能追加するとき、品質ゲートを明確化したいときに使用する。
---
