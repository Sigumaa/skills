...
2. 各部品に `type`（implementation/review/test/commit）を付与する。
3. 各部品に `depends_on` を列挙し、依存先が空なら並列候補とする。
4. 同一機能の品質ゲートは `implementation -> review -> test -> commit` の順で必ず依存線を張る。
5. 競合リスクがある部品は、依存がなくても直列化して理由を記録する。
6. PMはクリティカルパスと待機キューを分けて管理する。
