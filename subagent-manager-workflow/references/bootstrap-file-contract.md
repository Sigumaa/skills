...
- Plan modeで確定した内容を実装可能な粒度へ展開した正本。
- 実装、レビュー、テスト、コミット委譲はこのファイルの該当セクション参照を必須とする。
- 生成条件: ファイル欠落時。
- 更新条件: Plan mode内容との不整合、または `proposed-plan-contract.md` の必須項目欠落時。
...
- 不足があれば必要最小限の差分で補完する。
- `.codex/PROPOSED_PLAN.md` は委譲開始前に作成し、関連タスクIDを `PLAN_PROGRESS` と同期する。
